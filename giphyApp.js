$(document).ready(function () {
  ...
  $('#giphy-buttons').on("click", "button", function () {
    ...
    $.ajax({
      url: giphyUrl,
      method: "GET"
      // returning of the data from the url 
      ...
      for (var i = 0; i < results.length; i++) {
        ...
        p.add("button");
        p.attr("class", "favGiph");
        p.attr("data-title", results[i].title);
        console.log(p)
        var giphImage = $("<img>");
      ...
      $(document).on('click', '.favGiph', function () {
        // $('#fav').text("Favorite Giphs")
        favs.push($(this).attr('data-title'))
        $('#fav').append(favs)

        $(this).removeClass("favGiph");
      })
